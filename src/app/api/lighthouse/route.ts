...
export async function POST(request: NextRequest) {
  try {
    ...
    // Run Lighthouse CLI with headless Chrome flags
    const command = `"${lighthousePath}" "${url}" --output=json --output-path="${outputPath}" --quiet --chrome-flags="--headless --disable-gpu --no-sandbox --disable-dev-shm-usage --disable-background-timer-throttling --disable-backgrounding-occluded-windows --disable-renderer-backgrounding" --disable-device-emulation --throttling-method=devtools`;
